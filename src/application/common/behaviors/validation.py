...
class ValidationBehavior(Generic[TRequest]):
    ...
    async def handle(self, request: TRequest, next_: Callable[..., Coroutine]) -> Any:
        validation_result = self._validator.validate(request)
        if validation_result.is_valid:
            return await next_()
        errors: dict[str, list[str]] = validation_result.errors
...
class RegisterCommandValidationBehavior(ValidationBehavior[RegisterCommand]):
    ...
    async def handle(self, request: RegisterCommand, next_: Callable[..., Coroutine]) -> Any:
...
class LoginQueryValidationBehavior(ValidationBehavior[LoginQuery]):
    ...
    async def handle(self, request: LoginQuery, next_: Callable[..., Coroutine]) -> Any:
        return await super().handle(request, next_)
